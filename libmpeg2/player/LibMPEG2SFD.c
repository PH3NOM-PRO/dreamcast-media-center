...
static unsigned short AudioActive = 0;
    ...
    while (1)
    {
	    ...
	    case STATE_INVALID_END:
	        // Render Current Frame //
              ...
              {   
                  RenderActive=1;
                  RenderInit( info->sequence->width, info->sequence->height,
                              PVR_TXRFMT_YUV422 | PVR_TXRFMT_NONTWIDDLED,
                              AUDIO_CODEC_ADX, 1000.f/seqHdr->FrameRate, font );
                  switch(seqHdr->DAR)
                  {
                      case 2:
                           SetDisplay_4_3();
              ...
              RenderPush( info->display_fbuf->buf[0] );
...
#define DONEBYTES(x)		\
	...
	default:
        ...
        } else if (bytes > 0)
        {
            if(!AudioActive)
            {
                LibADX_Init( buf, bytes );   
                AudioActive=1;
            } 
            else LibADX_DecodeChunk( buf, bytes ); 
...
static void ps_loop (void)
{
    ...
    do {
          if(++frame%2==0)
             if(HandleInput((LibADX_Channels()==2) ? AICA_STEREO : AICA_MONO ))
                  break;
...
unsigned char LibMPEG2_DecodeSFD( char * mpeg_file, Font * f )
{
    printf("MPEG-DC: LibMPEG2 Program Stream Decoder\n");
    
    font = f;
       
    /* Open the MPEG File and Initialize LibMPEG2 */   
    in_file = fopen (mpeg_file, "rb");

    seqHdr = LibMPEG_ParseSequenceHeader(in_file);
    if(seqHdr==NULL)
    {
        printf("LibMPEG: Error Reading Sequence Header\n");
        return 0;
    }    
    else
    {
        printf("LibMPEG: %ix%i\n", seqHdr->Width, seqHdr->Height );
        printf("LibMPEG: FrameRate = %.3f\n", seqHdr->FrameRate );
    }
    
    mpeg2dec = mpeg2_init();
    if (mpeg2dec == NULL) return 0;
    
    mpeg2_malloc_hooks(malloc_hook, NULL);
    
    AudioActive = RenderActive = 0;
