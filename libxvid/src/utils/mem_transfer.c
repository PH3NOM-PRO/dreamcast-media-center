...
inline void copy_vector8( Vector8i *src, Vector8i *dst )
{
    *dst = *src;
}

void
transfer8x8_copy_sh4( uint8_t * const dst,
				      const uint8_t * const src,
				      const uint32_t stride)
{
	int j=8;
...
void
transfer8x4_copy_sh4( uint8_t * const dst,
				      const uint8_t * const src,
				      const uint32_t stride)
{
    int j;
	...
	for (j = 0; j < 4; j++)
    {
		s=(const uint32_t*)(src + j*stride);
		d=(uint32_t*)(dst + j*stride);
		d[0] = *(s++);
		d[1] = *(s++);
	}
}
